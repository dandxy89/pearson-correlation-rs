...
///
/// # Panics
pub fn corrcoef_f64x4(seq1: &[f64], seq2: &[f64]) -> f64 {
    let seq_len = seq1.len();
    assert_eq!(seq_len, seq2.len());

    let mut sum_sq1 = f64x4::splat(0.0);
    let mut sum_sq2 = f64x4::splat(0.0);
    ...
    for i in (0..seq_len).step_by(SIZE) {
        ...
        let vec1 = f64x4::from_slice_unaligned(&seq1[i..i + SIZE]);
        let vec2 = f64x4::from_slice_unaligned(&seq2[i..i + SIZE]);
